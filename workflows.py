...
from llama_index.core.tools import BaseTool, ToolSelection, FunctionTool, ToolOutput, ToolMetadata
...
class FunctionToolWithContext(FunctionTool):
    ...
    def from_defaults(
        ...
        if tool_metadata is None:
            ...
            signature_str = signature_str.replace("ctx: llama_index.core.workflow.context.Context, ", "")
...
class ConciergeAgent(Workflow):
    ...
    async def setup(
        ...
        if user_msg is None or agent_configs is None or llm is None or chat_history is None:
            raise ValueError("User message, agent configs, llm, and chat_history are required!")
    ...
    async def speak_with_sub_agent(
        ...
        system_prompt = agent_config.system_prompt.strip() + f"\n\nHere is the current user state:\n{user_state_str}"
        ...
        tool_calls: list[ToolSelection] = llm.get_tool_calls_from_response(response, error_on_no_tool_call=False)
        if len(tool_calls) == 0:
            chat_history.append(response.message)
            await ctx.set("chat_history", chat_history)
            return StopEvent(
                result={"response": response.message.content, "chat_history": chat_history}
            )
        ...
        for tool_call in tool_calls:
            if tool_call.tool_name == "RequestTransfer":
                ...
                ctx.write_event_to_stream(ProgressEvent(msg=f"Agent is requesting a transfer. Please hold."))
                return OrchestratorEvent()
            elif tool_call.tool_name in agent_config.tools_requiring_human_confirmation:
                ctx.write_event_to_stream(ToolRequestEvent(
    ...
    async def handle_tool_approval(
        ...
        if ev.approved:
            ...
            return ToolCallEvent(tools=agent_config.tools, tool_call=ToolSelection(tool_id=ev.tool_id, tool_name=ev.tool_name, tool_kwargs=ev.tool_kwargs))
        else:
            return ToolCallResultEvent(
                chat_message=ChatMessage(
                    role="tool", 
                    content=ev.response or "The tool call was not approved, likely due to a mistake or preconditions not being met."
                )
            )
    ...
    async def handle_tool_call(self, ctx: Context, ev: ToolCallEvent) -> ActiveSpeakerEvent:
        ...
        ctx.write_event_to_stream(ProgressEvent(msg=f"Tool {tool_call.tool_name} called with {tool_call.tool_kwargs} returned {tool_msg.content}"))
    ...
    async def orchestrator(
        ...
        if len(tool_calls) == 0:
            chat_history.append(response.message)
            return StopEvent(result={"response": response.message.content, "chat_history": chat_history})
        
        tool_call = tool_calls[0]
        selected_agent = tool_call.tool_kwargs["agent_name"]
        await ctx.set("active_speaker", selected_agent)

        ctx.write_event_to_stream(ProgressEvent(msg=f"Transferring to agent {selected_agent}"))
...
def get_stock_lookup_tools() -> list[BaseTool]:
    def lookup_stock_price(ctx: Context, stock_symbol: str) -> str:
        ...
        ctx.write_event_to_stream(ProgressEvent(msg=f"Looking up stock price for {stock_symbol}"))
...
def get_account_balance_tools() -> list[BaseTool]:
    async def get_account_id(ctx: Context, account_name: str) -> str:
        ...
        ctx.write_event_to_stream(ProgressEvent(msg=f"Looking up account ID for {account_name}"))
        user_state = await ctx.get("user_state")
    ...
    async def get_account_balance(ctx: Context, account_id: str) -> str:
        ...
        ctx.write_event_to_stream(ProgressEvent(msg=f"Looking up account balance for {account_id}"))
        user_state = await ctx.get("user_state")
...
def get_transfer_money_tools() -> list[BaseTool]:
    def transfer_money(ctx: Context, from_account_id: str, to_account_id: str, amount: int) -> str:
        """Useful for transferring money between accounts."""
        ctx.write_event_to_stream(ProgressEvent(msg=f"Transferring {amount} from {from_account_id} to account {to_account_id}"))
    ...
    async def balance_sufficient(ctx: Context, account_id: str, amount: int) -> bool:
        ...
        ctx.write_event_to_stream(ProgressEvent(msg="Checking if balance is sufficient"))
        user_state = await ctx.get("user_state")
    ...
    async def has_balance(ctx: Context) -> bool:
        ...
        ctx.write_event_to_stream(ProgressEvent(msg="Checking if account has a balance"))
        user_state = await ctx.get("user_state")
        return user_state["account_balance"] is not None and user_state["account_balance"] > 0
...
async def main():
    ...
    handler = workflow.run(user_msg="Hello!", agent_configs=agent_configs, llm=llm, chat_history=[], initial_state=initial_state)
        ...
        async for event in handler.stream_events():
            if isinstance(event, ToolRequestEvent):
                ...
                if "y" in approved.lower():
                    handler.ctx.send_event(ToolApprovedEvent(tool_id=event.tool_id, tool_name=event.tool_name, tool_kwargs=event.tool_kwargs, approved=True))
                else:
